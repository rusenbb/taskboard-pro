...
var TaskBoardView = class extends import_obsidian2.ItemView {
  ...
  setupDropZone(dropZone, config) {
    ...
    dropZone.addEventListener("drop", async (e) => {
      ...
      new import_obsidian2.Notice(`Moving task to ${config.name}...`);
      const success = await this.taskUpdater.moveTask(task, newStatus, isDoneColumn);
      if (success) {
        if (isDoneColumn && task.isRecurring) {
          new import_obsidian2.Notice("Recurring task completed - new instance created!");
        } else {
          new import_obsidian2.Notice(`Task moved to ${config.name}`);
        }
        await this.refresh();
      } else {
        new import_obsidian2.Notice("Failed to move task");
      }
      this.draggedTask = null;
    });
  }
  ...
  async archiveTask(task) {
    new import_obsidian2.Notice("Archiving task...");
    const success = await this.taskUpdater.archiveTask(task);
    if (success) {
      new import_obsidian2.Notice("Task archived");
      await this.refresh();
    } else {
      new import_obsidian2.Notice("Failed to archive task");
    }
  }
  formatDate(dateStr) {
    const today = /* @__PURE__ */ new Date();
...
var TaskBoardSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Include only these folders").setDesc("Comma-separated list of folders to scan. Leave empty to scan entire vault.").addText((text) => text.setPlaceholder("7-Kanban-Boards, Projects").setValue(this.plugin.settings.includeFolders.join(", ")).onChange(async (value) => {
      this.plugin.settings.includeFolders = value.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
      await this.plugin.saveSettings();
      await this.plugin.refreshBoard();
    }));
    new import_obsidian2.Setting(containerEl).setName("Excluded folders").setDesc("Comma-separated list of folders to exclude").addText((text) => text.setPlaceholder(".obsidian, templates").setValue(this.plugin.settings.excludeFolders.join(", ")).onChange(async (value) => {
      this.plugin.settings.excludeFolders = value.split(",").map((s) => s.trim());
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Include completed tasks").setDesc("Show completed tasks in the board").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeCompleted).onChange(async (value) => {
